import React from "react";

const Header = () => {
  return (
    <nav class="bg-transparent dark:bg-gray-900 fixed w-full z-20 top-0 start-0 xl:w-[1120px] xl:mx-[calc((100%-1120px)/2)] backdrop-blur-sm">
      <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4 xl:w-[1120px] xl:mx-[calc((100%-1120px)/2)] xl:flex xl:items-center xl:justify-evenly xl:flex-row xl:flex-wrap">
        <a href="/" class="flex items-center space-x-3 rtl:space-x-reverse">
          <img src="/logo-removebg.png" class="h-12" alt="Lara Logo" />
        </a>
        <div class="flex md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
          <div class="text-center md:mt-auto mt-2">
            <img
              src="data:image/jpeg;base64,"
              download="Francisco_Lara_Natoli_cv.pdf"
              class="size-14 bg-slate-50 rounded-full"
            ></img>
          </div>
          <button
            data-collapse-toggle="navbar-sticky"
            type="button"
            class="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-500 rounded-lg md:hidden"
            aria-controls="navbar-sticky"
            aria-expanded="false"
          >
            <span class="sr-only">Open main menu</span>
            <svg
              class="w-5 h-5"
              aria-hidden="true"
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 17 14"
            >
              <path
                stroke="currentColor"
                stroke-linecap="round"
                stroke-linejoin="round"
                stroke-width="2"
                d="M1 1h15M1 7h15M1 13h15"
              ></path>
            </svg>
          </button>
        </div>
        <div
          class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1"
          id="navbar-sticky"
        >
          <ul class="bg-purple-950 bg-opacity-45 flex flex-col p-4 md:p-0 mt-4 font-medium border border-purple-700 md:bg-transparent rounded-lg md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
            <li>
              <a href="#about" class="block py-2 px-3 text-white rounded">
                About
              </a>
            </li>
            <li>
              <a href="#experience" class="block py-2 px-3 text-white rounded">
                Experience
              </a>
            </li>
            <li>
              <a href="#projects" class="block py-2 px-3 text-white rounded">
                Projects
              </a>
            </li>
            <li>
              <a href="#contact" class="block py-2 px-3 text-white rounded">
                Contact
              </a>
            </li>
          </ul>
        </div>
      </div>
    </nav>
  );
};

export default Header;
